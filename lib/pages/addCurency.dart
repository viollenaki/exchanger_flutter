...
class _AddCurrencyScreenState extends State<AddCurrencyScreen> {
  ...
  Future<void> _fetchCurrencies() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showSnackBar('Ошибка загрузки валют. Попробуйте позже.', Colors.red);
      }
    } catch (e) {
  ...
  void _validateCode(String code) {
    ...
    setState(() {
      ...
      if (_isValidCode) {
        ...
        // Уведомляем Flutter об изменении текста
        _searchController.value = TextEditingValue(
          text: existingCurrency[1],
          selection: TextSelection.collapsed(offset: existingCurrency[1].length),
        );
      }
    });
  }
  ...
  void _selectCurrency(dynamic currency) {
    setState(() {
      ...
      _searchController.value = TextEditingValue(
        text: currency[1],
        selection: TextSelection.collapsed(offset: currency[1].length),
      );
      _codeController.value = TextEditingValue(
        text: currency[2],
        selection: TextSelection.collapsed(offset: currency[2].length),
      );
    });
  }
